// scripts/dom_shim.ts
var appStub = { innerHTML: "", addEventListener() {
} };
var store = {};
globalThis.document = {
...
function boardHtml() {
  ...
  return s.board.map((itemId, idx) => {
    ...
    const it = ITEMS[itemId];
    return `<div class="cell" data-cell="${idx}" style="--c:${it.color}">
...
function leftPanelHtml() {
  ...
  const roomsHtml = area.rooms.map((room) => {
    ...
    const tasksHtml = room.tasks.map((t) => {
      ...
      const btn = done ? "\u2713" : isCur ? `<button class="mini-btn" data-action="task" data-id="${t.id}" data-room="${room.id}" ${enough ? "" : "disabled"}>\u4EA4\u4ED8</button>` : "\u{1F512}";
...
// scripts/dom_smoke.ts
try {
  start();
  const appStub2 = globalThis.appStub;
  const html = appStub2.innerHTML;
